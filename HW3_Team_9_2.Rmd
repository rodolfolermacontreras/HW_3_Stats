...
```{r}
logistc_reg_model <- glm(
    Germ / Total ~ Extract + Seeds + Extract*Seeds,
    weights = Total, 
    family = "binomial",
    data = germ
)

logistc_reg_model_summary <- summary(logistc_reg_model)
logistc_reg_model_summary
```
```{r}
# Coefficients.
logistc_reg_model$coefficient # Additive effect on log-odds.
exp(logistc_reg_model$coefficient) # Multiplicative effect on odds.
```

```{r}
# Deviance goodness of fit test.
pchisq(
    logistc_reg_model_summary$deviance,
    logistc_reg_model_summary$df.residual,
    lower.tail = FALSE
)
```
...
```{r}
par(mfcol = c(2, 2))
logistc_reg_model_q_resid <- qresid(logistc_reg_model)
logistc_reg_model_q_resid_std <- (
    logistc_reg_model_q_resid / sqrt(1 - hatvalues(logistc_reg_model))
)
```

```{r}
scatter.smooth(
    logistc_reg_model_q_resid_std ~ asin(sqrt(fitted(logistc_reg_model))),
    col = "blue",
    las = 1,
...
```{r, message = FALSE}
scatter.smooth(
    logistc_reg_model_q_resid_std ~ germ$Extract,
    col = "blue",
    las = 1,
    ylab = "Standardized quantile residuals",
    xlab = "Extract",
...
```{r, message = FALSE}
scatter.smooth(
    logistc_reg_model_q_resid_std ~ germ$Seeds,
    col = "blue",
    las = 1,
    lwd = 2,
    ylab = "Standardized quantile residuals",
...
```{r, message = FALSE}
scatter.smooth(
    logistc_reg_model_q_resid_std ~ (germ$Seeds*germ$Extract),
    col = "blue",
    las = 1,
    lwd = 2,
    ylab = "Standardized quantile residuals",
...
qqnorm(logistc_reg_model_q_resid_std, las = 1, pch = 19, cex.lab = 1.5)
qqline(logistc_reg_model_q_resid_std, lty = "dashed")
